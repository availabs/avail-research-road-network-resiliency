...
def get_clipped_transcom_events_data(
    ...
    logger.info(f"Processing GIS dataset from: {source_path}")
    if layer_name:
    ...
    logger.info("Reading TRANSCOM Events features using bounding box filter...")
    try:
    ...
    if points_in_bbox_gdf.empty:
        logger.info("No found within the bounding box. Returning empty GeoDataFrame.")
    ...
    logger.info(
        f"Processing complete. Returning {len(final_gdf)} TRANSCOM events features."
    )
