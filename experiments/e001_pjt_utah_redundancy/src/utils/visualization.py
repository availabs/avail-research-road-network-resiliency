...
def render_detour_map(
    ...
    if viz_plot and not show_plot:
        raise BaseException(
            "INVARIANT BROKEN: If viz_plot passed in, the caller must call plt.close(f)."
        )

    render_either_path = (
        base_path_edges_gdf is not None or detour_path_edges_gdf is not None
    )
    focused_edge_color = "red" if render_either_path else "black"

    f, ax = viz_plot or plt.subplots(figsize=(10, 10), constrained_layout=True)
    ...
    basemap_sources = [
        ("Hillshade", cx.providers.Esri.WorldShadedRelief),
        ("Street Map", cx.providers.OpenStreetMap.Mapnik),
    ]

    cx.add_basemap(
        ax,  #
        crs=map_crs,
        source=basemap_sources[0][1],
        attribution="",
        alpha=0.8,
    )
    cx.add_basemap(
    ...
    cx.add_attribution(
        ax=ax,
        text=(
            f"{basemap_sources[0][0]}: "
            + basemap_sources[0][1].attribution
            + "\n"
            + f"{basemap_sources[1][0]}: "
            + basemap_sources[1][1].attribution,
        ),
    )
...
def render_tutorial_detour_map_series_for_edge(
    roadways_gdf,  #
    detour_info_df,
    edge_id,
    out_fpath_base=None,
    ...
    render_detour_map(
        out_fpath=f"series.edge-only.{out_fpath_base}",  #
        show_plot=show_plot,
        edge_gdf=detour_dfs["edge_gdf"],
    )

    render_detour_map(
        out_fpath=f"series.edge-with-origin-and-destination.{out_fpath_base}",  #
        show_plot=show_plot,
        edge_gdf=detour_dfs["edge_gdf"],
        origin_point_gdf=detour_dfs["origin_point_gdf"],
        destination_point_gdf=detour_dfs["destination_point_gdf"],
    )

    render_detour_map(
        out_fpath=f"series.base-path.{out_fpath_base}",  #
        show_plot=show_plot,
        edge_gdf=detour_dfs["edge_gdf"],
        origin_point_gdf=detour_dfs["origin_point_gdf"],
        base_path_edges_gdf=detour_dfs["base_path_edges_gdf"],
    )

    render_detour_map(
        out_fpath=f"series.detour-path.{out_fpath_base}",  #
        show_plot=show_plot,
        edge_gdf=detour_dfs["edge_gdf"],
        origin_point_gdf=detour_dfs["origin_point_gdf"],
        detour_path_edges_gdf=detour_dfs["detour_path_edges_gdf"],
    )


def show_gdfs(region_gdfs, colors=None, alpha=None):
    ...
    cx.add_basemap(
        ax,  #
        crs=region_gdfs[0].crs,
        source=cx.providers.OpenStreetMap.Mapnik,  # Add Mapbox tiles
    )
