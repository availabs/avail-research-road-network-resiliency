...
| `road_flooded_total_duration_hours`  | Real          | The total duration in hours this road segment was affected by "road_flooded" conditions, after merging all overlapping event times for this category.                              |
| `road_repairs_events_ids`            | String (List) | A sorted list of unique TRANSCOM event IDs related to "road_repairs" (where `road_repairs` flag was true, from washout repairs) affecting this segment.                            |
| `road_repairs_incidents_count`       | Integer64     | The count of distinct, continuous time periods during which washout repairs affected this road segment.                                                                            |
| `road_repairs_total_duration_hours`  | Real          | The total duration in hours this road segment was affected by washout repairs, after merging all overlapping event times for this category.                                        |
